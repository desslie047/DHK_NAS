...
def get_args(args=None):
    ...
    # 在构建 ArgumentParser 的位置添加：
    parser.add_argument("--robustness_seed", type=int, default=3407, help="Noise RNG seed")

    args = parser.parse_args(args)

    # post
    # 在 get_args() 返回前做边界校验（放在已有的 post-process 之后或附近）：
    args.noise_feat_std = max(0.0, float(args.noise_feat_std))
    args.noise_edge_drop = min(max(0.0, float(args.noise_edge_drop)), 0.99)
    args.noise_label_flip = min(max(0.0, float(args.noise_label_flip)), 0.99)
