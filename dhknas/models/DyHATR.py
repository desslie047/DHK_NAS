        ...
        for ttype in intra_features.keys():
            for ntype in self.ntypes:
                ...
                if len(types_features) > 0:
                    ...
                    # 处理空列表的情况，创建适当维度的零张量
                    first_graph = graphs[ttype]
                    # 获取当前节点类型的节点数量
                    if ntype in first_graph.x_dict:
                        batch_size = first_graph.x_dict[ntype].size(0)
                        device = first_graph.x_dict[ntype].device
                    else:
                        # 如果当前节点类型不存在，使用默认值
                        batch_size = 1
                        device = next(iter(first_graph.x_dict.values())).device

                    feature_dim = self.n_hid  # 使用hidden dimension
                    num_edge_types = 1  # 至少创建一个维度
                    types_features = torch.zeros(batch_size, num_edge_types, feature_dim, device=device)
