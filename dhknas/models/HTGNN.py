...
class HTGNNLayer(nn.Module):
    ...
    def forward(self, graphs, node_features):
        ...
        for ttype in self.timeframe:
            for ntype in self.ntypes:
                # 修改 HTGNNLayer 类的 forward 方法中的这部分代码
                types_features = []
                ...
                if len(types_features) == 0:
                    ...
                    # 假设我们需要保持与其他特征相同的形状
                    # 这里需要确定正确的节点数量，暂时使用1
                    zero_shape = (1, self.n_hid)  # 调整为适合您模型的形状
                    inter_features[ntype][ttype] = torch.zeros(zero_shape, device=device)
                    continue  # 跳过后续的stack操作
...
from dhknas.models.HLinear import HLinear
